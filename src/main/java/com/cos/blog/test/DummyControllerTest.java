...
public class DummyControllerTest {
	...
	// save 함수는 id를 전달하고 해당 id에 대한 데이터가 없으면 insert를 해요.
	// email, password
	...
	public User updateUser(@PathVariable int id, @RequestBody User requestUser) { // json 데이터를 요청 => Java Object(MessageConverter의 Jackson 라이브러리가 변환해서 받아줘요.)
		...
		// 더티 체킹
		// 수정이 생겼으므로 자동으로 DB에 Flush (업데이트 실행)
		return user;
	}
	...
	public User detail(@PathVariable int id) {
		// user/4를 찾으면 내가 데이터베이스에서 못 찾아오게 되면 user가 null이 되지 않니?
		// 그럼 return null이 되자나... 그럼 프로그램에 문제가 있지 않겠니...?
		// Optional로 너의 User 객체를 감싸서 가져올테니 null인지 아닌지 판단해서 return 하렴.
...
//		});
		...
		// 요청 : 웹브라우저
		// user 객체 = 자바 오브젝트
		// 변환 (웹브라우저가 이해할 수 있는 데이터) -> json (과거 : Gson 라이브러리)
		// 스프링부트 = MessageConverter라는 애가 응답 시에 자동 작동
		// 만약에 자바 오브젝트를 리턴하게 되면 MessageConverter가 Jackson 라이브러리를 호출해서
		// user 오브젝트를 브라우저에게 던져줍니다.
		return user;
	}
